...
require("./utils/uploadmedia.php");
require("./utils/read_write_functions.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (count($structuredFiles)) {
    foreach ($structuredFiles as $structuredFile) {
      ['id' => $uniqueImgID, 'imageName' => $imageName] = saveImagesToTempLocation($structuredFile);
    }
  }
  ...
  if ($uniqueImgID) {
    ...
    $response["url"] = DOMAIN_URL . 'images/' . $imageName;
    $response["msg"] = null;
